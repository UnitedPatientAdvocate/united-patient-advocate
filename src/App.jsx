...
@media (max-width: 580px) {
  ...
  .mobile-row-label:first-of-type { border-top: 0; }
  .comp-row {
    display: block;
    border-top: 0;
    background: #fff;
  ...
  .comp-cell.diy { border-bottom: 1px solid var(--stone); }
  .comp-cell.our-col {
    background: rgba(47,122,79,0.06);
    box-shadow: none;
  }
  .comp-cell-icon { width: 24px; height: 24px; flex: 0 0 24px; margin-top: 1px; }
  .comp-cell-text {
    flex: 1;
    min-width: 0;
    max-width: 100%;
  ...
  .comp-cta-cell.ours-verdict { box-shadow: none; }
  .comp-below { align-items: stretch; padding: 18px; }
  .btn-comp-cta { width: 100%; text-align: center; padding: 13px 18px; }
}
  ...
  <div class="hero-inner">
    ...
    <div class="hero-stats">
      ...
      <div class="hero-stat">
        ...
        <div class="hero-stat-l">to complete your<br>billing review submission</div>
...
<section class="analysis-section" style="padding-top:52px;">
  <div class="section-inner" style="max-width:720px">
    <div class="section-eyebrow eyebrow-navy" style="margin-bottom:10px;">Sample billing review report</div>
    ...
    <div class="analysis-card">
      ...
      <div class="analysis-lock-overlay">
        <div style="display:inline-flex;align-items:center;gap:7px;background:var(--greenL);border:1px solid rgba(22,101,52,0.2);border-radius:20px;padding:6px 16px;margin-bottom:14px;">
          ...
          <span style="font-size:0.68rem;font-weight:700;color:var(--green);letter-spacing:0.08em;">Your billing review documentation has already been prepared.</span>
        </div>
        <div class="lock-text">
  ...
  <div class="section-inner">
    ...
    <div class="steps-row">
      ...
      <div class="step-item">
        ...
        <div class="step-title">We prepare your billing review documentation</div>
  ...
  <div class="close-inner">
    ...
    <div class="upload-card">
      ...
      <div class="privacy-row">
        ...
        <div class="privacy-text"><strong>Your document is handled with strict confidentiality.</strong> We do not store, share, or sell your bill or personal information. Your file is used solely to prepare your billing review documentation and is not retained after your session.</div>
      </div>
...
// ─── RESULTS SCREEN ───────────────────────────────────────────────────────────
function Results({ results, userEmail, userName, mode, toggleMode }) {
  ...
  const rColor = summary.riskLevel==="HIGH"?(dark?"#E07070":"#C0392B"):(dark?"#FCD34D":"#92400E");
  const rBg    = summary.riskLevel==="HIGH"?(dark?"rgba(224,112,112,0.12)":"#FEF2F0"):(dark?"rgba(252,211,77,0.12)":"#FFF8EC");
        ...
        {!unlocked ? (
          <div style={{ background:surface,border:`1px solid ${border}`,borderRadius:18,padding:"36px 28px",borderTop:`4px solid ${navyC}`,marginBottom:28,textAlign:"center",boxShadow:"0 1px 3px rgba(0,0,0,0.06),0 4px 16px rgba(0,0,0,0.07)" }}>
            ...
            <div style={{ marginBottom:6 }}>
              <span style={{ fontSize:13,color:ink3,textDecoration:"line-through",marginRight:8 }}>Was $197</span>
          ...
          <div style={{ marginBottom:28 }}>
            ...
            {tab==="letter" && (
              <div style={{ background:surface,border:`1px solid ${border}`,borderRadius:18,overflow:"hidden" }}>
                <div style={{ background:navyC,padding:"15px 22px" }}>
                  <div style={{ fontFamily:"'Playfair Display',Georgia,serif",fontSize:16,fontWeight:800,color:"#fff" }}>Your Dispute Letter</div>
                  <div style={{ fontSize:12,color:"rgba(255,255,255,0.55)",marginTop:2 }}>Print and mail or email to billing department</div>
                </div>
                ...
                <div style={{ padding:"24px 28px",whiteSpace:"pre-wrap",lineHeight:2,fontSize:14,color:ink2,maxHeight:400,overflowY:"auto" }}>{disputeLetter}</div>
              </div>
            )}
            {tab==="script" && (
              <div style={{ background:surface,border:`1px solid ${border}`,borderRadius:18,overflow:"hidden" }}>
                <div style={{ background:navyC,padding:"15px 22px" }}>
                  <div style={{ fontFamily:"'Playfair Display',Georgia,serif",fontSize:16,fontWeight:800,color:"#fff" }}>Phone Script</div>
                  <div style={{ fontSize:12,color:"rgba(255,255,255,0.55)",marginTop:2 }}>Word-for-word — read directly when you call</div>
                </div>
                <div style={{ padding:"12px 22px",borderBottom:`1px solid ${border}`,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                ...
                <div style={{ padding:"24px 28px",whiteSpace:"pre-wrap",lineHeight:2,fontSize:14,color:ink2,maxHeight:400,overflowY:"auto" }}>{phoneScript}</div>
              </div>
            )}
            {tab==="action" && (
              <div>
            ...
            {tab==="rights" && (
              <div style={{ background:surface,border:`1px solid ${border}`,borderRadius:18,padding:"24px 26px" }}>
                <h3 style={{ fontFamily:"'Playfair Display',Georgia,serif",fontSize:18,fontWeight:800,color:ink,marginBottom:16,letterSpacing:"-0.02em" }}>Your Federal Billing Rights</h3>
                {yourRights?.map((r,i)=>{
                  const parts=r.split(":");
                  return (
...
// ─── ERROR SCREEN — replaces browser alert (Issue 23) ────────────────────────
...
export default function App() {
  ...
  const analyze = async () => {
    ...
    const prompt = `You are the AI engine behind United Patient Advocate. Analyze this patient bill and return ONLY valid JSON.
...
Return exactly this JSON structure with no markdown:
{
  "summary": {
    ...
    "errorsFound": ["First concern","Second concern","Third concern"],
    "keyFindings": "2-3 empowering sentences about what was found."
  },
  "disputeLetter": "Full formal dispute letter ready to print and mail.",
  "phoneScript": "Full word-for-word phone script.",
  "actionPlan": [
    {"step":1,"title":"Title","description":"Description","timeframe":"TODAY","powerTip":"Tip"},
    {"step":2,"title":"Title","description":"Description","timeframe":"Within 2 Days","powerTip":"Tip"},
    {"step":3,"title":"Title","description":"Description","timeframe":"Within 1 Week","powerTip":"Tip"},
    {"step":4,"title":"Title","description":"Description","timeframe":"Within 2 Weeks","powerTip":"Tip"},
    {"step":5,"title":"Title","description":"Description","timeframe":"Day 30","powerTip":"Tip"}
  ],
  "yourRights": [
    "Right to an Itemized Bill: Federal law requires hospitals to provide a complete itemized statement",
    ...
    try {
      ...
      setScreen("error"); // Proper error screen — no browser alert
    }
  };
  ...
  if (screen==="results")   return <Results results={results} userEmail={userEmail} userName={userName} {...shared}/>;
